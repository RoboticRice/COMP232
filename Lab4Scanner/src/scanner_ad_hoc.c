 ...
 * Date: 02/20/2019
 *
 * DO NOT USE, file was kept for ref
 *
**/
    ...
    while ((token->type == INVALID_TOKEN) && ((c = getchar()) != EOF))
    {
        switch (c)
        {
            ...
            case '0'...'9': // if c is a number
                ...
                return token;
            case '=':
                token->strVal = malloc((strlen("=")+1)*sizeof(char)); //size of Char is 1, so this can be abv as malloc(2)
                //strcpy(token->strVal, "="); //redundant, only one value
